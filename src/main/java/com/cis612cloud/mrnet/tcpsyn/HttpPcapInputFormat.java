...
public class HttpPcapInputFormat extends FileInputFormat<LongWritable, ObjectWritable> {
    ...
    public static PcapRecordReader initPcapRecordReader(Path path, long start, long length, Reporter reporter, Configuration conf) throws IOException {
        ...
        HttpPcapReader reader = initPcapReader(stream, conf);
        return new PcapRecordReader(reader, start, length, baseStream, stream, reporter);
    }

    public static HttpPcapReader initPcapReader(DataInputStream stream, Configuration conf) {
        try {
            Class<? extends HttpPcapReader> pcapReaderClass = conf.getClass(READER_CLASS_PROPERTY, HttpPcapReader.class, HttpPcapReader.class);
            Constructor<? extends HttpPcapReader> pcapReaderConstructor = pcapReaderClass.getConstructor(DataInputStream.class);
